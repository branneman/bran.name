...
const config = Object.freeze({
    ...
    glob: {
        ...
        pages: './src/pages/**/*',
        css: './src/static/css/!(_)*.css'
    },
    datafile: './dist/data.json'
});
 ...
 * @private
 */
gulp.task('_clean', cb => { rimraf(config.dist); cb(); });
...
gulp.task('_download-content', () => {
    ...
    const arr2obj = arr => arr.reduce((acc, curr) => { acc[curr.contentType] = curr.items; return acc }, {});
    const getContentTypeId = item => item.sys.id;
    const extractEntry = id => ({ items }) => ({ contentType: id, items });
    const getEntriesByType = id => client.getEntries({ content_type: id }).then(extractEntry(id));
    const getAllContentTypes = items => Promise.all(items.map(getContentTypeId).map(getEntriesByType));
    const writeJSONData = filename => data => fs.writeFileSync(filename, json(data, { encoding: 'utf8' }));
 ...
 * @private
 */
gulp.task('_remove-temporary-files', cb => { fs.unlinkSync(config.datafile); cb(); });
...
gulp.task('_render-templates', cb => {
    ...
    const render = (file, data) => nunjucks.render(file, data);
...
gulp.task('_render-pages', cb => {
    ...
    const getVinyl = filename => ({ filename, contents: fs.readFileSync(filename, 'utf8') });
    const render = data => vinyl => { vinyl.contents = nunjucks.render(vinyl.filename, { data }); return vinyl; };
    const mapFilename = filename => `${path.relative(`${config.src}/pages/`, filename)}`.replace(/\.njk$/, '.html');
    const mapToDist = vinyl => { vinyl.filename = mapFilename(vinyl.filename); return vinyl; };
    const writeDist = vinyl => fs.writeFileSync(`${config.dist}/${vinyl.filename}`, vinyl.contents);
...
gulp.task('_compile-css', () => {
    ...
    const getVinyl = filename => ({ filename, contents: fs.readFileSync(filename, 'utf8') });
    const renderCss = vinyl => processor.process(vinyl.contents, { from: vinyl.filename });
    const render = vinyl => renderCss(vinyl).then(result => { vinyl.contents = result.css; return vinyl; });
    const mapFilename = filename => `${path.relative(`${config.src}/static/css/`, filename)}`;
    const mapToDist = vinyl => { vinyl.filename = mapFilename(vinyl.filename); return vinyl; };
    const writeDist = vinyl => fs.writeFileSync(`${config.dist}/static/css/${vinyl.filename}`, vinyl.contents);
